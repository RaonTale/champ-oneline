...
(() => {
  ...
  window.CHAMP = {
    ...
    //
    // ▷ 도감 카드용 표시 필드(선택) — 넣으면 카드에 뜬다. 계산엔 영향 없음.
    //   ja 일본명 · genus 분류(예 '흉조포켓몬') · gender 성별(-1 무성,0 수컷100%,1 87.5%수…8 암컷100%)
    //   flavor 설명문 · heightm 키(m) · pid 스프라이트 번호(PokeAPI, 예 한카리아스 445)
    //   (종족값·타입·무게·특성은 위 계산용 값을 카드도 그대로 공유 → 중복 입력 불필요)
    //
    ...
    // ── 도감 카드 설명 채우기 ────────────────────────────────────────────────
    // 생성된 dex.js 에 설명이 빈 항목(주로 9세대)을 여기서 한글명으로 채운다. 즉시 반영.
    //   '한글명': '설명문'   (여기 값이 dex.js 보다 우선)
    abilityNotes: {
      // '예리함': '베기 기술의 위력이 1.5배가 된다.',
      // '고대활성': '부스트에너지를 지녔거나 날씨가 쾌청일 때, 가장 높은 능력치가 올라간다.',
      // '황금몸': '상태 이상과 상대의 변화 기술을 막는다.',
    },
    moveNotes: {
      // '기가톤해머': '거대한 망치를 힘껏 휘둘러서 공격한다. 다음 턴에는 이 기술을 쓸 수 없다.',
      // '아쿠아스텝': '경쾌한 스텝으로 상대를 농락하며 공격한다. 자신의 스피드를 올린다.',
    },
